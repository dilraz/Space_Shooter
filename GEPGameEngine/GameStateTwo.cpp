...
void GameStateTwo::Enter()
{
	bgSpriteTex = Game::Instance()->LoadTexture("background2.png");
...
void GameStateTwo::Exit()
{
	//to-do:: de-allocate memory, texture, surface etc, sound 
	this->~GameStateTwo();
}

void GameStateTwo::CheckCollision()
{
	...
	for (int b = 0; b < (int)player->GetBullets().size(); b++)
	{
		...
		for (int i = 0; i < (int)asteroids.size(); i++)
		{
				...
				))
			{
				...
				if (gone == 4)
				{
					Game::Instance()->GetFSM()->PushState(new GameStateThree(lives, score));
