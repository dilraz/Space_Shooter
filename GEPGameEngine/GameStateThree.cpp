...
void GameStateThree::Enter()
{
	bgSpriteTex = Game::Instance()->LoadTexture("background3.png");
...
void GameStateThree::Exit()
{
	//to-do:: de-allocate memory, texture, surface etc, sound 
	this->~GameStateThree();
}

void GameStateThree::CheckCollision()
{
		...
		for (int i = 0; i < (int)asteroids.size(); i++)
		{
				...
				))
			{
				...
				if (gone ==4)
				{
					string g = "You Won!";
					RenderFont(true, g.c_str(), 500, 50);
					Game::Instance()->GetFSM()->PushState(new GameWonState());
				}
